...
import { SelectGenreStyled, SelectTextStyled, TopMoviesWrapperStyled } from './Moive.styles';
...
const TopMovies = () => {
  ...
  const handleGet = async (): Promise<string[]> => {
    try {
      let response;
      if (options_genre.params?.genre == 'all') response = await axios.request(options_all);
      else response = await axios.request(options_genre);
  ...
  const [movies, setMovies] = useState<MovieDataFull[]>([
    top1,
    top2,
    top3,
    top4,
  ...
  return (
    <TopMoviesWrapperStyled>
      <SelectGenreStyled>
        <SelectTextStyled>Trending films by genre:</SelectTextStyled>
        <SelectMenu options={genresOptions} onChange={handleGenreChange} />
      </SelectGenreStyled>
      <Movies movies={movies} moviesFull={movies} />
    </TopMoviesWrapperStyled>
  );
};
